...
- **Eval harness (M01 ✅):** Aptness evaluator (`evaluate_aptness.py`) shipped — salience-weighted Jaccard over shared cluster_ids, calibrated against MUNCH inapt controls (CC BY 4.0, 10,261 apt + 1,447 inapt). Combined V2 baseline recorded at `data-pipeline/output/eval_baseline_v2.json` (MRR 0.0073, separation 0.0103, aptness_rate 0.0849). Separation magnitude below the 0.3 target — closing it is downstream tuning work in M02+.
...
*(REQUIREMENTS.md previously lived under `.gsd/` — preserved verbatim at [archive/2026-05-03-gsd-final-snapshot.md](../archive/2026-05-03-gsd-final-snapshot.md). The forward-going capability contract should be re-stated here when next revised.)*

## Milestone Sequence

See [PIPELINE.md](PIPELINE.md) for current status. The originally-planned five-milestone sequence:

1. **M01 — Automated Eval Harness** ✅ — discriminative aptness evaluator + parameter sweep, replacing MRR as primary KPI.
2. **M02 — Asymmetric Ortony Scoring** — vehicle-side salience weighting in forge scoring.
3. **M03 — Cascade Gate-and-Rank** — concreteness gate → Ortony rank → domain distance re-rank.
4. **M04 — Type-Aligned Structural Matching** — property type diversity bonus in scoring.
5. **M05 — Novelty Tracking** — MuseScorer-style dynamic buckets + creative yield curve.

Programme rationale and ordering: see [M01-eval-harness-context.md](M01-eval-harness-context.md) — the programme overview was originally written as M01's context document.
