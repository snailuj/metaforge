...
func TestCascadeParity_GoMatchesPythonGroundTruth(t *testing.T) {
	...
	// Parity crib was generated by Python with the M03 config (hard gate,
	// Alpha=1.0, DCap=0.77, no OrtonyWeight/ConcretenessBonusCoef, no soft
	// gate). DefaultCascadeConfig now carries loop-tuned production values —
	// pin the crib config explicitly so this test stays anchored to the
	// Python ground truth, not the production defaults.
	cfg := forge.CascadeConfig{
		ConcretenessThreshold: 1.0,
		Alpha:                 1.0,
		DCap:                  0.77,
		Composition:           forge.CompositionAdditive,
		GateMode:              forge.GateModeHard,
		// OrtonyWeight and ConcretenessBonusCoef are zero here, which EvaluateCascadePair
		// treats as identity (1.0) and disabled respectively — matching the Python crib.
		Mode:          forge.ModeCluster,
		EmbeddingDMin: 0.4,
		EmbeddingDMax: 0.85,
		EmbeddingTopK: 100,
	}
