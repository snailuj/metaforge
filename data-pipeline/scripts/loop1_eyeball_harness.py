...
For each topic in ``loop1_eyeball_topics.json``:
  ...
  2. Re-score each candidate via Python ``evaluate_cascade_pair`` using
     PRODUCTION_CASCADE_CONFIG (which inherits the live-metaphor shipping
     dataclass defaults: alpha=1.0, d_cap=0.77, rerank_exponent=0.12,
     concreteness_bonus_coef=0.0, ortony_weight=1.0, gate_mode=soft,
     gate_alpha=3.0).
...
def render_report(results: list[dict]) -> str:
    ...
    out.append("")
    out.append(
        "Candidate set + ranking generated by Go `/forge/suggest` vs Python "
        "`evaluate_cascade_pair` with `PRODUCTION_CASCADE_CONFIG` (inherits "
        "the 2026-05-27 live-metaphor shipping defaults: alpha=1.0, "
        "d_cap=0.77, rerank_exponent=0.12, concreteness_bonus_coef=0.0, "
        "ortony_weight=1.0, gate_mode=soft, gate_alpha=3.0). Both sides "
        "score with the same config; differences here surface picker/"
        "lookup-driven shifts, not config drift."
    )
    out.append("")
    out.append(
